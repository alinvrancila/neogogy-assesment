...
{
  const web = fs.readFileSync(path.join(process.cwd(), 'src', 'components', 'humanAdvantage', 'ResultCover.tsx'), 'utf-8');
  const css = fs.readFileSync(path.join(process.cwd(), 'src', 'app', 'humanAdvantage.css'), 'utf-8');
