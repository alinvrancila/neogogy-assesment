...
import { BRAND, CORE_QUESTION, ECOSYSTEM, NEXT_STEP, fileStem } from '@/brand';
import { PRIVACY, TERMS } from '@/content/legal';
import { applicableItems, compute } from '@/engine';
import { evidenceFor } from '@/engine/content';
import { optionsFor, canShuffle } from '@/components/humanAdvantage/items';
...
head('The four organisations, as supplied and linked');
{
  ...
  const res = fs.readFileSync(path.join(process.cwd(), 'src', 'components', 'humanAdvantage', 'Results.tsx'), 'utf-8');
...
head('The page draws the continuum the way the report does');
{
  ...
  ok('the band comes from the report\'s own route module',
    /from '@\/components\/humanAdvantage\/ascent\/route'/.test(fig));
...
head('The site says what it does with your data');
{
  ...
  const gate = fs.readFileSync(path.join(process.cwd(), 'src', 'components', 'humanAdvantage', 'Results.tsx'), 'utf-8');
  ...
  ok('and how to be rid of them', /clearing site data/i.test(prose));
  {
    const app = fs.readFileSync(
      path.join(process.cwd(), 'src', 'components', 'humanAdvantage', 'HumanAdvantageApp.tsx'), 'utf-8');
  ...
  const results = fs.readFileSync(path.join(process.cwd(), 'src', 'components', 'humanAdvantage', 'Results.tsx'), 'utf-8');
...
head('The page counts what the assessment actually asks');
{
  ...
  const app = fs.readFileSync(path.join(process.cwd(), 'src', 'components', 'humanAdvantage', 'HumanAdvantageApp.tsx'), 'utf-8');
...
head('The file a respondent receives is named after the product');
{
  // It was called Neogogy_Formation_Compass.pdf long after the Formation
  // Compass stopped existing, because the string was typed into two routes and
  // an email attachment rather than taken from the name. Derived now, so the
  // next rename cannot leave it behind.
  ok('the filename comes from the product name',
    fileStem() === 'Neogogy_Human_Advantage_Report', fileStem());
  ok('and follows the name if the name changes',
    fileStem().includes(BRAND.org) && BRAND.report.split(/\s+/).every((w) => fileStem().includes(w)),
    `${fileStem()} against "${BRAND.org}" and "${BRAND.report}"`);
  ok('a participant copy is still a legal filename',
    /^[A-Za-z0-9_]+$/.test(fileStem("Ana Maria O'Brien")), fileStem("Ana Maria O'Brien"));

  const typed: string[] = [];
  for (const f of FILES.concat([path.join(process.cwd(), 'src', 'lib', 'email.ts')])) {
    const body = fs.readFileSync(f, 'utf-8');
    for (const m of body.matchAll(/filename="([^"$][^"]*)"/g)) {
      // A literal filename with no interpolation in it is a name waiting to go
      // stale. Group exports are allowed their own descriptive names.
      if (/\.pdf$/i.test(m[1])) typed.push(`${path.relative(process.cwd(), f)}: ${m[1]}`);
    }
  }
  ok('no route hardcodes a report filename', typed.length === 0, typed.join(', '));
}
...
head('No answer refers to an option that may not have been shown');
{
  ...
  ok('the safety net that catches chained copy is still in place',
    /buildsOnThePrevious/.test(fs.readFileSync(
      path.join(process.cwd(), 'src', 'components', 'humanAdvantage', 'items.tsx'), 'utf-8')));
