...
const DIR = path.join(process.cwd(), "docs", "humanAdvantage", "sources", "pastor");
...
if (!corpus.length) {
  ...
  console.log("Put them in docs/humanAdvantage/sources/pastor/originals and run scripts/ingest-sources.sh.");
...
const closings = fs.readFileSync(
  path.join(process.cwd(), "src", "components", "humanAdvantage", "PastorModules.tsx"), "utf-8");
