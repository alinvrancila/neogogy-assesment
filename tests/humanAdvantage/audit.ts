...
head('P0-2: the contradiction the scoring acts on is shown to the reader');
{
  ...
  const screen = fs.readFileSync(
    path.join(process.cwd(), 'src', 'components', 'humanAdvantage', 'Results.tsx'), 'utf-8');
...
head('B3: a report has its own address, and the address is a secret');
{
  ...
  const view = fs.readFileSync(
    path.join(process.cwd(), 'src', 'components', 'humanAdvantage', 'ReportView.tsx'), 'utf-8');
...
head('P0-4 The first step comes from the constraint, not from a template');
{
  ...
  const card = fs.readFileSync(
    path.join(process.cwd(), 'src', 'components', 'humanAdvantage', 'ascent', 'modules.tsx'), 'utf-8');
...
head('7.4 Every spoke and bar reads one direction');
{
  const viz = fs.readFileSync(
    path.join(process.cwd(), 'src', 'components', 'humanAdvantage', 'Visuals.tsx'), 'utf-8');
