...
export function bandRulerScene(width = 430, compact = false): Scene {
  const W = width, H = compact ? 40 : 74;
  const y = compact ? 17 : 34;
  ...
  for (const [from, to, colour, label] of bands) {
    ...
    p.push(text({ x: (x(from) + x(to)) / 2, y: y + 3, size: 8.5, anchor: 'middle', fill: C.ink, text: label }));
  }
  p.push(line({ x1: x(45), y1: y - 11, x2: x(45), y2: y + 11, stroke: C.vulnerability, strokeWidth: 1, dash: [2, 2] }));
  for (const v of [0, 40, 65, 100]) {
    p.push(text({ x: x(v), y: y + 18, size: 8, mono: true, fill: C.mute,
      anchor: v === 0 ? 'start' : v === 100 ? 'end' : 'middle', text: String(v) }));
  }
  if (!compact) {
    p.push(text({ x: x(45), y: y + 30, size: 8.5, anchor: 'middle', fill: C.vulnerability,
...
export function medianIllustrationScene(width = 430): Scene {
  ...
  values.forEach((v, i) => {
    ...
    p.push(text({ x: cx, y: cy + 20, size: 7.5, anchor: 'middle', mono: true, fill: C.mute, text: String(v) }));
  });
  p.push(callout({ x: startX + 4 * gap, y: 16, toX: startX + 4 * gap, toY: 25,
    text: 'the median', anchor: 'middle' }));
  p.push(text({ x: 0, y: H - 2, size: 8, fill: C.mute,
...
export function miniRouteScene(centreStage: number, width = 200): Scene {
  ...
  p.push(text({ x: 0, y: 29, size: 7.5, fill: C.mute, text: 'stage 1' }));
  p.push(text({ x: W, y: 29, size: 7.5, fill: C.mute, anchor: 'end', text: 'stage 10' }));
...
export function stackedBandScene(
  ...
  for (const [v, colour, label] of segs) {
    ...
    if (w > 0) {
      ...
      if (w > 13) p.push(text({ x: x + w / 2, y: barY + 10, size: 9.5, anchor: 'middle', mono: true, fill: C.ink, text: String(v) }));
    }
    x += w;
  }
  p.push(text({ x: 0, y: barY + barH + 11, size: 8, fill: C.mute,
    text: `strong ${d.strong} · developing ${d.developing} · watch ${d.watch}` }));
  if (d.vulnerable > 0) {
    p.push(text({ x: W, y: barY + barH + 11, size: 8, anchor: 'end', fill: C.vulnerability,
      text: `${d.vulnerable} at or below 45` }));
  }
  p.push(callout({ x: W, y: barY - 3, toX: W - 4, toY: barY + 2,
    text: `${d.n} people`, anchor: 'end' }));
  return {
...
export function useBarScene(rows: Array<{ label: string; n: number; share: number }>, n: number, width = 430): Scene {
  const W = width, rowH = 22, top = 30;
  const barX = 112, barW = W - barX - 70;
  const p: Prim[] = [];
  const max = Math.max(1, ...rows.map((r) => r.n));
  rows.forEach((r, i) => {
    const y = top + i * rowH;
    p.push(text({ x: 0, y: y + 2, size: 9.5, fill: C.ink, text: r.label }));
    const w = Math.max(r.n ? 2 : 0, (r.n / max) * barW);
    p.push(rect({ k: 'rect', x: barX, y: y - 6, w, h: 12, r: 2, fill: C.strength, opacity: 0.45 }));
    p.push(text({ x: W, y: y + 2, size: 9.5, anchor: 'end', mono: true, fill: r.n ? C.ink : C.hair,
      text: `${r.n} of ${n}` }));
  });
