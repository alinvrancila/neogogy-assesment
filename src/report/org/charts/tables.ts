...
import { C, bandColour, callout, fitChars, one, rect, text, whole, wrap, type Prim, type Scene } from '../scene';
...
export function registerScene(rows: RegisterRow[], n: number, width = 510): Scene {
  const W = width, rowH = 32, top = 46;
  const p: Prim[] = [];
  p.push(text({ x: 0, y: 24, size: 8, fill: C.mute, text: 'exposure' }));
  p.push(text({ x: 136, y: 24, size: 8, fill: C.mute, text: 'capability' }));
  p.push(text({ x: 256, y: 24, size: 8, fill: C.mute, text: 'people at or below 45' }));
  p.push(text({ x: 392, y: 24, size: 8, fill: C.mute, text: 'the practice that addresses it' }));
  rows.forEach((r, i) => {
    const y = top + i * rowH;
    p.push(rect({ k: 'rect', x: 0, y: y - 8, w: W, h: rowH - 4, r: 3,
      fill: i % 2 ? C.hairSoft : C.white, opacity: i % 2 ? 0.35 : 1 }));
    p.push(text({ x: 4, y: y + 4, size: 8.5, fill: C.ink, text: r.exposure.slice(0, 24) }));
    p.push(text({ x: 136, y: y + 4, size: 8.5, fill: C.mute, text: r.dimension.slice(0, 24) }));
    ...
    p.push(text({ x: 326, y: y + 4, size: 8.5, mono: true, fill: C.ink, text: `${r.atOrBelow} of ${n}` }));
    p.push(text({ x: 392, y: y + 4, size: 8, fill: C.mute, text: r.practice.slice(0, 30) }));
  });
...
export function segmentScene(panels: SegmentPanel[], width = 510): Scene {
  const W = width, ch = 56, gap = 8, cols = 2;
  const cw = (W - gap) / cols;
  const p: Prim[] = [];
  panels.forEach((s, i) => {
    ...
    p.push(text({ x: x + 9, y: y + 14, size: 9.5, weight: 600, fill: s.shown ? C.ink : C.mute, text: s.cut.slice(0, 38) }));
    if (s.shown) {
      p.push(text({ x: x + 9, y: y + 27, size: 8, fill: C.mute,
        text: `${s.n} people · median ${one(s.median ?? 0)} · ${s.modalStage ?? ''}`.slice(0, 46) }));
      if (s.constraint) p.push(text({ x: x + 9, y: y + 38, size: 8, fill: C.gate, text: `held most by ${s.constraint}`.slice(0, 46) }));
    } else {
      // A withheld cut carries no size, only what it would take to show it.
      p.push(text({ x: x + 9, y: y + 27, size: 8, fill: C.mute,
        text: 'Withheld, because this cut is small enough' }));
      p.push(text({ x: x + 9, y: y + 36, size: 8, fill: C.mute,
...
export function baselineScene(reason: string, width = 470): Scene {
  ...
  p.push(text({ x: 14, y: 22, size: 12, weight: 600, fill: C.ink, text: 'This is your baseline' }));
  p.push(text({ x: 14, y: 38, size: 8.5, fill: C.mute, text: reason.slice(0, 96) }));
  const tail = reason.slice(96, 192);
  if (tail.trim()) p.push(text({ x: 14, y: 48, size: 8.5, fill: C.mute, text: tail }));
  // Two empty rulers, showing what a second wave would fill in.
  for (let i = 0; i < 2; i++) {
    const y = 64 + i * 14;
    p.push(text({ x: 14, y: y + 4, size: 8, fill: C.mute, text: i === 0 ? 'this wave' : 'next wave' }));
    p.push(rect({ k: 'rect', x: 72, y: y - 3, w: W - 100, h: 7, r: 3,
      fill: i === 0 ? C.strength : C.hair, opacity: i === 0 ? 0.35 : 0.4 }));
    if (i === 1) p.push(text({ x: W - 24, y: y + 4, size: 7.5, anchor: 'end', fill: C.mute, text: 'not yet taken' }));
  }
...
export function waveScene(rows: Array<{ name: string; before: number; after: number }>, width = 470): Scene {
  const W = width, rowH = 24, top = 30;
  const barX = 150, barW = W - barX - 70;
  const p: Prim[] = [];
  rows.forEach((r, i) => {
    const y = top + i * rowH;
    const up = r.after >= r.before;
    p.push(text({ x: 0, y: y + 2, size: 9.5, fill: C.ink, text: r.name.slice(0, 30) }));
    ...
    p.push(text({ x: W, y: y + 2, size: 8.5, anchor: 'end', mono: true, fill: up ? C.strengthDeep : C.watch,
