...
export default function HumanAdvantageApp({ initialPersona }: { initialPersona?: Persona }) {
  ...
  const [result, setResult] = useState<HumanAdvantageResult | null>(null);
  ...
  const resultRef = useRef<HumanAdvantageResult | null>(null);
  ...
  useEffect(() => {
    ...
    try {
      const at = { assessment: true, screen, pos };
      if (window.history.state?.assessment
  ...
  useEffect(() => {
    const onPop = (e: PopStateEvent) => {
      const at = e.state as { assessment?: boolean; screen?: Screen; pos?: number } | null;
      if (!at?.assessment) return;
  ...
  const finish = useCallback(() => {
    ...
    if (process.env.NODE_ENV !== 'production') {
      ...
      if (stray.length) {
        ...
        console.error('[assessment] answers contain non-applicable item ids:', stray);
      }
    }
  ...
  const submitGate = useCallback(async (data: GateData) => {
    ...
    try {
      ...
      setResult(payload.result as HumanAdvantageResult);
      resultRef.current = payload.result as HumanAdvantageResult;
  ...
  /* ---------------------------------------------------------------- render */
  // Every selector in humanAdvantage.css is scoped under .nfc, so each screen must be
