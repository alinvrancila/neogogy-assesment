...
**Both new fields are on `HumanAdvantageResult`** (`riskRegister`, `ninetyDayPlan`) and
...
measure: it never touches stage, archetype, or score, and `HumanAdvantageResult` carries
...
`tests/humanAdvantage/group.ts` recomputes every statistic by hand from the same
