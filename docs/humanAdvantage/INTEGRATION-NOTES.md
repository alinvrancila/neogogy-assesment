...
**Test relocation.** `tests/` became `tests/humanAdvantage/`. Import specifiers rewritten from
...
(`tsx tests/humanAdvantage/synthetic.ts`). `tsx` added as a devDependency.
...
**Verification.** `tests/humanAdvantage/branches.ts` added: it asserts, for all four personas at all five
...
**The refactor is provably prose-preserving.** `tests/humanAdvantage/dump.ts` generates reports for 60
...
**Scoring location.** The results screen renders a `HumanAdvantageResult` returned by `POST /api/submit`.
...
persona (the screen list itself is verified exhaustively by `tests/humanAdvantage/branches.ts`, but the
...
**Cause.** Every selector in `src/app/humanAdvantage.css` is scoped under `.nfc`, which exists so the design
system never leaks into the Tailwind based `/admin` route. The v1 `HumanAdvantageApp` returned a single
...
- **Persona union** is declared once in `src/engine/types.ts` and enumerated by
  ...
  (`api/submit`, `api/report`), the assessment UI (`HumanAdvantageApp`), the dev
...
in `docs/humanAdvantage/sources/pastor/`. They were not present when the persona was
...
`docs/humanAdvantage/sources/pastor/README.md` lists the documents and names the two
