...
| Cover field | Source |
|---|---|
| persona | `HumanAdvantageResult.persona`, mapped through `COVER_PERSONA` (administrator becomes leader, pastor becomes minister) |
...
`src/components/humanAdvantage/ResultCover.tsx` renders the same six compositions in
...
styles live under `.rcover` in `src/app/humanAdvantage.css`: one shared grammar (brand,
