...
- `tests/humanAdvantage/audit.ts` ties each fix to its numbered audit finding.
- `tests/humanAdvantage/placement.ts` holds the scoring guarantees across all seven sets.
...
checked by `tests/humanAdvantage/link.ts` against what the code does.
...
`tests/humanAdvantage/contrast.ts` computes the ratios on every release.
...
- **Grep tests are worth what you paid for them.** Every source-text assertion in
  ...
  thing instead. `tests/humanAdvantage/link.ts` now drives the real route handlers, the
...
- **The regression snapshot is the safety net** for any engine change:
  `npx tsx tests/humanAdvantage/dump.ts` before and after, and diff.
